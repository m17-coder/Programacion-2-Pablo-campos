...
void crearProducto(Tienda* tienda){
    ...
    if (respuesta == 'S' || respuesta == 's') {
        ...
        strcpy(tienda->productos[tienda->numProductos].fechaRegistro, buffer);
        tienda->numProductos++;
    }
}
...
void listarProductos(Tienda* tienda){
    ...
    cout << "╔════╦═══════════╦══════════════════╦══════════════╦═══════╦════════╦════════════╗" << endl;
    cout << "║ ID ║  Código   ║      Nombre      ║  Proveedor   ║ Precio║ Stock  ║    Fecha   ║" << endl;
    cout << "╠════╬═══════════╬══════════════════╬══════════════╬═══════╬════════╬════════════╣" << endl;
    for (int i = 0; i < tienda->numProductos; i++) {
    ...
    if(prov != nullptr) {
        ...
        cout << "║ " << setw(2) << prod->id
             ...
             << " ║ " << setw(16) << prod->nombre
             << " ║ " << setw(12) << nombreProv
             << " ║ " << setw(5) << fixed << setprecision(2) << prod->precio
             << " ║ " << setw(6) << prod->stock
...
void crearProveedor(Tienda* tienda){
    ...
    cout << "ID de provedor: " << tienda->siguienteIdProveedor << endl;
    cout << "Nombre: " << nombre << endl;
    cout << "RIF: " << rif << endl;
    ...
    if (respuesta == 'S' || respuesta == 's') {
        ...
        strcpy(tienda->proveedores[tienda->numProveedores].fechaRegistro, buffer);
...
void listarProveedores(Tienda* tienda){
    ...
    cout << "╠════╦══════════════════╦══════════════╦══════════════╦════════╦════════╦══════╣" << endl;
    cout << "║ ID ║      Nombre      ║     RIF      ║   Teléfono   ║ Email  ║   Dir  ║ F.R  ║" << endl;
    cout << "╠════╬══════════════════╬══════════════╬══════════════╬════════╬════════╬══════╣" << endl;
    ...
    for (int i = 0; i < tienda->numProveedores; i++) {
        ...
        contadoractivos++;
        cout << "║ " << setw(2) << p->id << " ";
        cout << "║ " << setw(16) << left << p->nombre << " ";
        cout << "║ " << setw(12) << left << p->rif << " ";
        cout << "║ " << setw(12) << left << p->telefono << " ";
        cout << "║ " << setw(6) << left << p->email << " ";
        cout << "║ " << setw(6) << left << p->direccion << " ";
        if(strlen(p->fechaRegistro) == 0) {
            cout << "║ " << setw(4) << left << "N/A" << " ║" << endl;
        } else {
            cout << "║ " << setw(4) << left << p->fechaRegistro << " ║" << endl;
        }
    }
    cout << "╚════╩══════════════════╩══════════════╩══════════════╩════════╩════════╩══════╝" << endl;
    cout << "Total de proveedores activos: " << contadoractivos << " de " << tienda->numProveedores << " en sistema." << endl;
    pausarYlimpiarpantalla();
}
void eliminarProveedor(Tienda* tienda) {
    ...
    // Validación de integridad: No eliminar si tiene productos
...
void listarClientes(Tienda* tienda){
    ...
    cout << "╠════╦══════════════════╦══════════════╦══════════════╦════════╦════════╦══════╣" << endl;
    cout << "║ ID ║     Nombre       ║  Cédula/RIF  ║  Teléfono    ║ Email  ║ Dir    ║ F.R ║" << endl;
    cout << "╠════╬══════════════════╬══════════════╬══════════════╬════════╬════════╬══════╣" << endl;
    for (int i = 0; i < tienda->numClientes; i++) {
...
void registrarVenta(Tienda* tienda){
    ...
    if (respuesta == 'S' || respuesta == 's') {
        ...
        t->cantidad = cantidad;
        t->total = total;
        t->precioUnitario = precioUnitario;
        t->tipo = 'V';
        t->cancelada = false;
        strcpy(t->fecha, buffer);
        tienda->productos[i].stock -= cantidad;
...
void buscarTransacciones(Tienda* tienda){
    ...
    switch(opcion){
        ...
        case 4:
            ...
            break;
        case 5:
            cout << "Ingrese fecha (YYYY-MM-DD) (si desea cancelar presione -1): ";
...
void listarTransacciones(Tienda* tienda) {
    ...
    for (int i = 0; i < tienda->numTransacciones; i++) {
        ...
        const char* nombreProd;
        if (idxP != tienda->numProductos) {
                nombreProd = tienda->productos[idxP].nombre;
            } else {
